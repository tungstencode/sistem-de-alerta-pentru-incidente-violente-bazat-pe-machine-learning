...
class AddProjectPage extends Component {
  ...
  async componentDidMount() {
    try {
      // fetch data
      let req = await axios.get('/camera');
      const cameras = req.data.map(camera => ({label: team.name, value: team.id}));
  ...
  handleEdit = deliverable =>
    this.setState({isFormOpen: true, deliverableToEdit: deliverable});
        ...
        <Loader isLoading={isLoading}>
          <Formik
              ...
              deliverables: Yup.array()
                .of(
                  Yup.object().shape({
                    ...
                    weight: Yup.number(t('Errors.Number'))
                      .positive(t('Errors.Pozitive'))
                      .min(1, t('Errors.Subunit'))
                      .max(100, t('Errors.Subunit'))
                      .required(t('Errors.Required')),
                    deadline: Yup.date()
                      .min(new Date(), t('Errors.MinDate', {date: new Date()}))
                      .required(t('Errors.Required')),
                  })
                )
                .required(t('Errors.Required'))
                .min(
                  ...
                  t('Errors.Min', {
                    value: 1,
                    name: t('Projects.Add.Deliverables'),
                  })
                )
                .max(
                  6,
            ...
            onSubmit={async finalData => {
              const {name, deliverables, summary, team, professor} = finalData;
            ...
            {({errors, values, setFieldValue}) => (
              <Form>
                ...
                <div className={classes.deliverablesHeader}>
                  <div
                    className={
                      errors.deliverables ? classes.deliverableError : ''
                    }>
                    ...
                    {errors.deliverables ? (
                      <Typography
                        variant="body2"
                        className={classes.deliverableErrorText}>
                        {JSON.stringify(errors.deliverables)}
                      </Typography>
                    ) : null}
                  </div>
                      ...
                      <Grid item xs={6} md={4} justify="space-between">
                        <Card>
                          ...
                          <CardActions disableSpacing>
                            <IconButton
                              aria-label={t('Projects.Add.Deliverable.Edit')}
                              onClick={() =>
                                console.log(deliverable) ||
                                this.handleEdit(deliverable)
                              }>
                              <EditIcon />
                            </IconButton>
                            <IconButton
                              aria-label={t('Projects.Add.Deliverable.Delete')}
                              onClick={() => {
